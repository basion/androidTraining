...
import cn.com.hakim.viewpagertest.view.PagerAdapter;
 ...
 * ViewPager在onTouch时，不允许父view对事件拦截，如果需要父view拦截事件，就要重写viewpager
 * 当水平滑动事件在viewpager滑动边界产生时，重新分发事件。事件序列是由action_down开始，如果down事件没有被父view拿到，事件序列内其他事件都不会被父view拿到处理。
 * 所以需要重新分发事件，并产生一个假的down事件。
 *
 * 尚有一个bug未处理
 */
...
public class ViewPager extends ViewGroup {
    ...
    /**
     * 该方法在dispatchTouchEvent中调用，dispatchTouchEvent调用子view的dispatchTouchEvent方法，所以，该方法决定了事件是是否向子view中传递。
     * @param ev
     * @return
     */
    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == ACTION_CANCEL || action == MotionEvent.ACTION_UP) {
            // Release the drag.
...
//                    requestParentDisallowInterceptTouchEvent(true);//// TODO: 2017/4/10
...
//                    requestParentDisallowInterceptTouchEvent(true);//// TODO: 2017/4/10
    ...
    /**
     * onTouch是本view事件的处理的最后调用的方法，对于viewgroup，如果子view返回true，那么
     * @param ev
     * @return
     */
    @Override
...
//                        ViewParent parent = getParent();//// TODO: 2017/4/7  设置允许父view拦截事件
//                        if (parent != null) {
...
//                        }
                ...
                if (mIsBeingDragged) {
                    ...
                    //TODO 拖拽添加
                    int direction = wantLeftOrRight(x);
                    if (direction == 1||direction==2){
                        final long now = SystemClock.uptimeMillis();
                        final MotionEvent cancelEvent = MotionEvent.obtain(now, now,
                                MotionEvent.ACTION_CANCEL, 0.0f, 0.0f, 99);
                        ((CDragLayout)getParent()).dispatchDown(cancelEvent);
                        final MotionEvent event = MotionEvent.obtain(now, now, MotionEvent.ACTION_DOWN, ev.getX(activePointerIndex), ev.getY(activePointerIndex), 99);
                        ((CDragLayout)getParent()).dispatchDown(event);
                        Log.e(TAG,"event.metaState = "+event.getMetaState());
                        return false;
                    }
                    //End
    ...
    private int wantLeftOrRight(float x){
        ...
        int direction = 0;//中间状态，没有滑动到左右边缘
        if (scrollX < leftBound) {//往左滑动
            direction = 1;//往左1
        } else if (scrollX > rightBound) {//往右滑动
            direction = 2;//往右2
        }
        return direction;
    }
